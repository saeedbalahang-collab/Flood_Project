...
def ensure_main_raw_dataset(target_path: str = None) -> str:
    """
    Download the main raw dataset (RepData.zip) from GitHub and place it
    at the path the original code expects:
    BASE_DIR/Results/ML_Preparation/Final_ML_Dataset.csv
    """
    if target_path is None:
        target_path = os.path.join(BASE_DIR, "Results", "ML_Preparation", "Final_ML_Dataset.csv")

    if os.path.exists(target_path):
        print(f"[OK] Raw main dataset already present: {target_path}")
        return target_path

    extract_dir = os.path.join(BASE_DIR, "_downloads", "RepData")
    download_and_extract_zip(github_raw_url(RAW_MAIN_DATA_ZIP), extract_dir)

    csv_path = find_first_csv(extract_dir)
    os.makedirs(os.path.dirname(target_path), exist_ok=True)
    shutil.copy(csv_path, target_path)
    print(f"[OK] Copied {csv_path} -> {target_path}")
    return target_path
